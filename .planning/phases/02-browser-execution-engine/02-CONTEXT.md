...
- **D-08 (added after research review):** Login is performed by a new deterministic script (`scripts/ui-login.mjs`, Playwright core, not interactive MCP tool calls) that reads `QA_AGENT_UI_USER`/`QA_AGENT_UI_PASSWORD` directly from `process.env`, submits the login form, and exports `storageState` to disk. The orchestrator never sees the plaintext password as a tool-call argument — same credential-isolation guarantee as Phase 1's `QA_AGENT_TOKEN` (D-06). Playwright MCP (D-01) is still used for the *post-login* autonomous flow navigation (D-03's "no per-project setup" philosophy applies there); only the credential-entry step itself is script-driven. — **Reversibility:** costly — switching to MCP-driven login later means re-deriving the credential-isolation guarantee and touching the hook/gate wiring built around the script boundary.
